...
std::string ItemStack::getName() {
	std::string str;
	SymCall<std::string&>("?getRawNameId@ItemStackBase@@QEBA?AV?$basic_string@DU?$char_traits@D@std@@V?$allocator@D@2@@std@@XZ",
		this, &str);
	return str;
}
...
std::vector<ItemStack*> Container::getSlots() {
	std::vector<ItemStack*> s;
