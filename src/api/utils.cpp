...
void registerCommand(const string& name, const string& desc,
                     const py::function& cb, CommandPermissionLevel perm) {
...
PlayerClass getPlayer(const string& target /*name or xuid*/) {
  ...
  if (p == nullptr) throw std::runtime_error("Failed to find player");
  return p;
}
...
BlockClass getBlock(const BlockPos& pos, int dim) {
  if (Global<Level> == nullptr) throw std::runtime_error("Level is not loaded");
  Block* b = Level::getBlockEx(pos, dim);
  if (b == nullptr) throw py::value_error("Failed to find block");
...
void explode(const Vec3& pos, int dim, float power, bool destroy, float range,
  ...
  if (!bs) throw py::value_error("Unknown dimension ID");
...
void setSignBlockMessage(BlockPos pos, int dim, const string& text) {
  BlockSource* bs = Level::getBlockSource(dim);
  if (bs == nullptr) throw py::value_error("Unknown dimension ID");
  auto sign =
      static_cast<SignBlockActor*>(Global<Level>->getBlockEntity(pos, dim));
...
bool sendCmdOutput(const string& output) {
  ...
  SymCall(
      "??$_Insert_string@DU?$char_traits@D@std@@_K@std@@YAAEAV?$basic_ostream@"
      "DU?$char_traits@D@std@@@0@AEAV10@QEBD_K@Z",
      std::ostream&, std::ostream&, const char*,
      size_t)(std::cout, finalOutput.c_str(), finalOutput.size());
  return true;
}
