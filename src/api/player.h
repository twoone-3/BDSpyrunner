...
struct PlayerClass {
  ...
  pair<BlockPos, int> getRespawnPosition();
  ...
  bool setSidebar(const string& title, vector<pair<string, int>> data,
                  ObjectiveSortOrder order);
  bool removeSidebar();
  bool setBossBar(int64_t uid, const string& name, float percent,
  ...
  bool sendSimpleForm(const string& title, const string& content,
                      ...
                      const vector<string>& images, const py::function& cb);
  ...
  BlockClass getBlockFromViewVector(bool includeLiquid = false,
  ...
  bool simulateDestory(const BlockPos& pos,
  ...
  bool simulateInteract(const BlockPos& pos,
                        ScriptModuleMinecraft::ScriptFacing face);
  bool simulateJump();
  ...
  bool simulateNavigateTo(vector<Vec3> path, float speed = 1.f);
