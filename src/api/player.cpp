...
py::dict NavigateResultToObject(
    const ScriptModuleMinecraft::ScriptNavigationResult& res) {
  py::dict obj;
...
Vec3 PlayerClass::getPos() {
  if (thiz == nullptr) return Vec3::ZERO;
  return thiz->getPosition();
}

BlockPos PlayerClass::getBlockPos() {
  if (thiz == nullptr) return BlockPos::ZERO;
...
float PlayerClass::getSpeed() {
  if (thiz == nullptr) return 0.f;
  return thiz->getSpeed();
}

Vec2 PlayerClass::getDirection() {
  if (thiz == nullptr) return Vec2::ZERO;
...
BlockClass PlayerClass::getBlockStandingOn() {
  if (thiz == nullptr) return {{}, 0};
  return {thiz->getBlockPosCurrentlyStandingOn(nullptr),
          thiz->getDimensionId()};
}
...
pair<BlockPos, int> PlayerClass::getRespawnPosition() {
  if (thiz == nullptr) return {};
  auto position = thiz->getRespawnPosition();
  if (position.first.y == 32767) {
    auto region = Level::getBlockSource(position.second);
    if (region) position.first = region->getHeightmapPos(position.first);
  }
  return position;
}
...
bool PlayerClass::setSidebar(const string& title,
...
bool PlayerClass::setBossBar(int64_t uid, const string& name, float percent,
                             BossEventColour colour) {
  if (thiz == nullptr) return {};
  if (percent < 0)
    percent = 0;
  else if (percent > 100)
  ...
  thiz->updateBossEvent(uid, name, value,
                        colour);  // TODO: test whether it work
  return thiz->sendBossEventPacket(BossEvent::Show, name, value, colour);
}

bool PlayerClass::removeBossBar() {
  if (thiz == nullptr) return {};
  return thiz->sendBossEventPacket(BossEvent::Hide, "", 0,
...
bool PlayerClass::sendCustomForm(const string& str, const py::function& cb) {
  if (thiz == nullptr) return {};
  Player* p = thiz;
  return thiz->sendCustomFormPacket(str, [p, cb](const string& arg) {
...
bool PlayerClass::sendSimpleForm(const string& title, const string& content,
  ...
  if (buttons.size() != images.size())
    throw py::value_error(
        "The number of buttons is not equal to the number of images");
  Player* p = thiz;
...
bool PlayerClass::sendModalForm(const string& title, const string& content,
...
BlockClass PlayerClass::getBlockFromViewVector(bool includeLiquid,
                                               bool solidOnly,
                                               float maxDistance,
...
SimulatedPlayer* PlayerClass::asSimulatedPlayer() {
  if (thiz->isSimulatedPlayer()) return static_cast<SimulatedPlayer*>(thiz);
  return nullptr;
}
...
py::dict PlayerClass::simulateNavigateTo(const EntityClass& target,
                                         float speed) {
  SimulatedPlayer* sp = asSimulatedPlayer();
  if (!sp) return {};
  Actor* actor = target.thiz;
...
bool PlayerClass::simulateNavigateTo(vector<Vec3> path, float speed) {
  SimulatedPlayer* sp = asSimulatedPlayer();
  if (!sp) return {};
  sp->simulateNavigateToLocations(move(path), speed);
  return true;
}
...
bool PlayerClass::simulateUseItem(const ItemClass& item, const BlockPos& pos,
...
bool PlayerClass::simulateUseItem(int slot, const BlockPos& pos,
                                  ScriptModuleMinecraft::ScriptFacing face,
                                  const Vec3& relativePos) {
  SimulatedPlayer* sp = asSimulatedPlayer();
  if (!sp) return {};
...
bool PlayerClass::simulateInteract(const BlockPos& pos,
                                   ScriptModuleMinecraft::ScriptFacing face) {
  SimulatedPlayer* sp = asSimulatedPlayer();
  if (!sp) return {};
...
bool PlayerClass::simulateDestory(const BlockPos& pos,
                                  ScriptModuleMinecraft::ScriptFacing face) {
  SimulatedPlayer* sp = asSimulatedPlayer();
  if (!sp) return {};
...
bool PlayerClass::removeItem(int inventory_id, int count) {
  ...
  if (inventory_id > container.getSize()) return false;
...
class SetActorDataPacket : public Packet {
 public:
  ActorRuntimeID rid;
  vector<unique_ptr<DataItem>> items;

  MCAPI virtual MinecraftPacketIds getId() const;
  MCAPI virtual string getName() const;
...
bool PlayerClass::setHeadShow(const string& name) {
  ...
  if (packet.rid.id == 0) return false;
