...
bool EconomySystem::init() {
  ...
  if (!money::get) logger.warn("Fail to load API money::getMoney!");

  money::set = (LLMoneySet_T)GetProcAddress(h, "LLMoneySet");
  if (!money::set) logger.warn("Fail to load API money::setMoney!");

  money::add = (LLMoneyAdd_T)GetProcAddress(h, "LLMoneyAdd");
  if (!money::add) logger.warn("Fail to load API money::addMoney!");

  money::reduce = (LLMoneyReduce_T)GetProcAddress(h, "LLMoneyReduce");
  if (!money::reduce) logger.warn("Fail to load API money::reduceMoney!");

  money::transfer = (LLMoneyTrans_T)GetProcAddress(h, "LLMoneyTrans");
  if (!money::transfer) logger.warn("Fail to load API money::transMoney!");

  money::getHistory = (LLMoneyGetHist_T)GetProcAddress(h, "LLMoneyGetHist");
  if (!money::getHistory) logger.warn("Fail to load API money::getTransHist!");

  money::clearHIstory =
      (LLMoneyClearHist_T)GetProcAddress(h, "LLMoneyClearHist");
  if (!money::clearHIstory) logger.warn("Fail to load API money::clearHist!");
...
string EconomySystem::getMoneyHist(xuid_t player, int time) {
  if (money::getHistory)
